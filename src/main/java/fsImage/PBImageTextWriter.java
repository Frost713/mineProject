...
//    );
//    RestHighLevelClient client = new RestHighLevelClient(
//            RestClient.builder(new HttpHost("10.177.36.168", 9200, "http"),
//                    new HttpHost("10.177.36.169", 9200, "http"))
//    );
     ...
     *                 in memory instead.
     */
    PBImageTextWriter(String tempPath) throws IOException {
//    this.out = out;
        if (tempPath.isEmpty()) {
    ...
    private void outputINodes(InputStream in) throws IOException {
        ...
        for (int i = 0; i < s.getNumInodes(); ++i) {
            ...
            try {
                ...
                String entry = getEntry(parentPath, p);
                String path = JSONObject.parseObject(entry).get("Path").toString();
                bulkRequest.add(new IndexRequest("hdfs_resolve_index").source(JSON.parseObject(entry),XContentType.JSON));
                if (count == 1000) {
